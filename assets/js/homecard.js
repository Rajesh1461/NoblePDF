...
function updateFavoritesSection() {
    const favoritesGroup = document.getElementById('groupFavorites');
    const favoritesContainer = favoritesGroup ? favoritesGroup.querySelector('.nav-group-container') : null;
    if (!favoritesContainer) {
        return; // Safely exit if favourites UI is not present
    }
...
function updateFavoritesView() {
    ...
    const favoritesVisibility = document.getElementById('favouritesVisibility');
    const favoritesList = JSON.parse(localStorage.getItem('favoritesList')) || [];

    if (!favoritesVisibility || !favoritesGroup) {
        return; // Safely exit if favourites UI not present on page
    }

    if (isFavoritesView && favoritesList.length > 0) {
        favoritesVisibility.style.display = 'flex';
        favoritesGroup.style.display = 'flex';
    } else {
        if (favoritesList.length > 0) {
            ...
            favoritesVisibility.style.display = 'none';
        }
    }
}
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        const response = await fetch('/assets/files/popularity.txt');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
