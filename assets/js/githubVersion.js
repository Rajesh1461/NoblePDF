...
function getDownloadUrl() {
    ...
    const baseUrl = 'https://files.Noblepdf.com/';

    // Determine file based on machine type and security
    if (machineType === 'Server-jar') {
        return baseUrl + (activeSecurity ? 'Noble-PDF-with-login.jar' : 'Noble-PDF.jar');
    }
...
function getTranslatedPriority(priority) {
    switch (priority?.toLowerCase()) {
        case 'urgent':
            return updatePriorityUrgent;
        case 'normal':
            return updatePriorityNormal;
        case 'minor':
            return updatePriorityMinor;
        case 'low':
...
async function getUpdateSummary() {
    // Skip remote update checks in local/dev contexts
    try {
        const isFile = window.location.protocol === 'file:';
        const host = (window.location && window.location.hostname) || '';
        const isLocal = isFile || host === 'localhost' || host === '127.0.0.1' || host === '';
        if (isLocal) {
            return null;
        }
    } catch (_) {}
    ...
    const url = `https://supabase.Noble.com/functions/v1/updates?from=${currentVersion}&type=${type}&login=${activeSecurity}&summary=true`;
    console.log("Fetching update summary from:", url);
    try {
        const response = await fetch(url);
        // console.debug("Supabase update check status:", response.status);
        if (response.status === 200) {
            const data = await response.json();
            return data;
        } else {
            // console.debug("Update summary unavailable (status):", response.status);
            return null;
        }
    } catch (error) {
        // console.debug("Skipping update summary (fetch failed):", error);
        return null;
    }
}
...
async function getFullUpdateInfo() {
    ...
    const url = `https://supabase.Noble.com/functions/v1/updates?from=${currentVersion}&type=${type}&login=${activeSecurity}&summary=false`;
    console.log("Fetching full update info from:", url);
    try {
        const response = await fetch(url);
        // console.debug("Supabase full update status:", response.status);
        if (response.status === 200) {
            const data = await response.json();
            return data;
        } else {
            // console.debug("Full update info unavailable (status):", response.status);
            return null;
        }
    } catch (error) {
        // console.debug("Skipping full update info (fetch failed):", error);
        return null;
    }
}

async function getCurrentVersionFromBypass() {
    const url = "https://raw.githubusercontent.com/Noble-Tools/Noble-PDF/master/build.gradle";
    try {
        const response = await fetch(url);
        if (response.status === 200) {
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable" role="document" style="max-height: 80vh;">
        <div class="modal-content" style="max-height: 80vh;">
          ...
          <div class="modal-footer">
            ...
            <a href="https://github.com/Noble-Tools/Noble-PDF/releases" target="_blank" class="btn btn-outline-primary">${updateViewAllReleases}</a>
